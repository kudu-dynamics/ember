...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = EmberFileSystem.MyFileSystemFactory.class)
public class EmberFileSystem implements GFileSystem {
	 ...
	 */
	public EmberFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + EmberFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<EmberFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public EmberFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			EmberFileSystem fs = new EmberFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
